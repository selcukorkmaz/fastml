...
model_reg <- fastml(
    data = pbc_baseline,
    label = "albumin",
    algorithms = c("rand_forest_survival", "xgboost_aft"),
...
> Survival naming: `xgboost_aft` is the AFT-only xgboost alias (legacy `xgboost` maps here). `rand_forest_survival` is the censored RF alias (legacy `rand_forest` maps here).
...
model_surv <- fastml(
  ...
  algorithms = c("cox_ph", "parametric_surv", "xgboost_aft"),
...
`fastml` handles missing data inside the preprocessing recipe so that the same
statistics learned during training are applied at prediction time.  Set the
`impute_method` argument to control which recipe step is added:

- `"medianImpute"` — Imputes numeric predictors using the median.
- `"knnImpute"` — Uses K-nearest neighbors to fill in missing values.
- `"bagImpute"` — Applies bagging-based imputation for complex relationships.
- `"remove"` — Drops rows containing missing predictor values.

Because the imputation logic lives in the recipe, the trained model can safely
score new datasets without recomputing statistics on the full training set.
